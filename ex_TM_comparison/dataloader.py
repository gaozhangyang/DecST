...
class MovingMNIST(data.Dataset):
    def __init__(self, root, is_train=True, n_frames_input=10, n_frames_output=10, num_objects=[2],
                 transform=None,require_back=False):
        super(MovingMNIST, self).__init__()

        self.dataset = None
        self.require_back=True
        if is_train:
        ...
        self.length = int(1e2)# if self.dataset is None else self.dataset.shape[1]
    ...
    def __getitem__(self, idx):
        ...
        if self.require_back:
            # b_mask = get_binary_mask(input)
            # H,W = b_mask.shape
            T,C,H,W=input.shape
            return input, output, torch.zeros(1,1,H,W)#b_mask.view(1,1,H,W)
        else:
            return input, output
...
def load_data(
    ...
    train_set = MovingMNIST(root=data_root, is_train=True, n_frames_input=10, n_frames_output=10, num_objects=[2],require_back=require_back)
    test_set = MovingMNIST(root=data_root, is_train=False, n_frames_input=10, n_frames_output=10, num_objects=[2],require_back=require_back)
