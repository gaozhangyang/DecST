...
from ex_caltech.conv import BasicConv1d, BasicConv2d, BasicConv3d, GroupConv2d
...
class ConvSC(nn.Module):
    def __init__(self, C_in,C_out, stride,  transpose=False):
        ...
        # self.conv = Inception_ST(C_in,C_out,stride,transpose)
...
#--------------------------- encoder
class EncoderShallowPlus(nn.Module):
    def __init__(self,C_in,C_hid):
        super(EncoderShallowPlus,self).__init__()
    ...
    def forward(self,x):# B*4, 3, 128, 128
        ...
        enc2 = self.enc2(enc1) #     (4,32,32,32)
        latent = self.enc3(enc2) #     (4,32,32,32)
...
#--------------------------- decoder
class DecoderShallowPlus(nn.Module):
    def __init__(self,C_in,C_hid,unet=True):
        super(DecoderShallowPlus,self).__init__()
        self.unet = unet
        if unet:
...
class ConvUnet(nn.Module):
    def __init__(self, dataname,shape_in,C_hid=16, T_hid=4):
        ...
        if dataname == 'traffic' or dataname == 'kitticaltech':
    ...
    def forward(self,x_raw,b_mask, Y_b=None,batch_y=None):
        if Y_b is not None:
            Y_b = Y_b.float()
        else:
            Y_b = 0
