...
from ex_kth.model import ConvUnet
...
class Exp_KTH(Exp_Basic):
    def __init__(self, args):
        super(Exp_KTH, self).__init__(args)
    ...
    def _get_data(self):
        ...
        self.train_loader, self.vali_loader, self.test_loader, self.data_mean, self.data_std = load_data(config['dataname'],
                    config['batch_size'], config['val_batch_size'], config['data_root'], 
    ...
    def _iter_batch(self, batch_x, batch_y, b_mask, is_valid=False, is_test=False):
        ...
        for patch in range(cnt):
            if is_test is False:
                ...
                cur_seq, fore, back, mask, loss2 = self.model(cur_seq, b_mask, return_bf=True)
                pred_y.append(cur_seq)
                fore_y.append(fore)
                back_y.append(back)
                mask_y.append(mask)
        ...
        if is_test is False:
            pred_y = torch.cat(pred_y, dim=1) if cnt > 0 else pred_y
            loss = criterion(pred_y, batch_y) + loss_sub * self.alpha if is_valid is False else criterion(pred_y, batch_y)
            return pred_y, loss
        else:
            pred_y = torch.cat(pred_y, dim=1) if cnt > 0 else pred_y
            fore_y = torch.cat(fore_y, dim=1) if cnt > 0 else fore_y
            back_y = torch.cat(back_y, dim=1) if cnt > 0 else back_y
            mask_y = torch.cat(mask_y, dim=1) if cnt > 0 else mask_y
            return pred_y, fore_y, back_y, mask_y
    ...
    def train(self, args):
        ...
        for epoch in range(config['epoch_s'], config['epoch_e']):
            ...
            for batch_x,batch_y,b_mask in train_pbar:
                ...
                _, loss = self._iter_batch(batch_x, batch_y, b_mask)
    ...
    def test(self,args):
        ...
        for batch_x,batch_y,b_mask in self.test_loader:
            ...
            pred_y, fore_y, back_y, mask_y = self._iter_batch(batch_x, batch_y, b_mask, is_valid=False, is_test=True)
            true = batch_y.detach().cpu().numpy()#.squeeze()
            
            inputs.append(batch_x.detach().cpu().numpy())
            trues.append(batch_y.detach().cpu().numpy())
            preds.append(pred_y.detach().cpu().numpy())
            Y_f.append(fore_y.detach().cpu().numpy())
            Y_b.append(back_y.detach().cpu().numpy())
            mask.append(mask_y.detach().cpu().numpy())
    ...
    #         pred_y, loss2 = self.model(batch_x,b_mask)#.squeeze()
