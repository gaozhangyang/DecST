...
from ex_sota.model import ConvUnet
...
class Exp_Traffic(Exp_Basic):
    def __init__(self, args):
        ...
        self.writer = SummaryWriter(self.path+'/logging')
        self._get_data()
    ...
    def vali(self, vali_loader, criterion, name,epoch):
        ...
        for i, (batch_x,batch_y, b_mask) in enumerate(vali_pbar):
            ...
            pred_y,loss2 = self.model(batch_x,b_mask)
        ...
        self.writer.add_scalars(name,{'mse':mse,'mae':mae,'rmse':rmse,'mape':mape,'mspe':mspe,'loss':total_loss},global_step=epoch)
        self.model.train()
    ...
    def train(self, args):
        ...
        for epoch in range(config['epoch_s'], config['epoch_e']):
            ...
            for batch_x,batch_y,b_mask in train_pbar:
                ...
                pred_y, loss2 = self.model(batch_x,b_mask)
                loss = criterion(pred_y, batch_y)+loss2*config['alpha']
    ...
    def test(self,args):
        ...
        for batch_x,batch_y,b_mask in self.test_loader:
            batch_x = batch_x.to(self.device)#.flip(dims=[1])
            batch_y = batch_y
            b_mask = b_mask.to(self.device)
